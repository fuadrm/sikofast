            ...
            <tbody>
                ...
                {{-- @dd($bb->no_po) --}}
                    <tr>
                        ...
                        <td><label class="label {{ ($bb->status_bb == 1) ? 'label-success':'label-warning'}}">{{ ($bb->status_bb == 1) 
                            ? 'Terpenuhi' : 'Belum Terpenuhi' }}</label></td>
                        <td>
                            @if($bb->status_bb == 1)
                                @if(in_array(auth()->user()->role,[1,3]))
                                <a href="/bahanbaku/status/{{$bb->id}}" class="btn btn-sm btn-warning">Belum Terpenuhi</a>
                                @endif
                            @else
                                @if(in_array(auth()->user()->role,[1,3]))
                                <a href="/bahanbaku/status/{{$bb->id}}" class="btn btn-sm btn-primary">Terpenuhi</a>
                                @endif
                            @endif
                        </td>
                        <td>
                            <a href="/bahanbaku/detail/{{$bb->id}}" class="btn btn-sm btn-success"><i class="nav-icon fas fa-eye"></i></a>
                            @if(in_array(auth()->user()->role,[1,3]))
                            <a href="/bahanbaku/edit/{{$bb->id}}" class="btn btn-sm btn-warning"><i class="nav-icon fas fa-edit"></i></a>
                            @endif
                            @if ($errors->any())
                            ...
                            <form action="/bahanbaku/destroy/{{$bb->id}}" method="POST" class="d-inline">
                                @method('delete')
                                @csrf
