                ...
                <form class="form-inline ml-3" action="/pemesanan" >
                    <div class="input-group input-group-sm float-lg-left">
                      <input class="form-control form-control-navbar" type="search" 
                        placeholder="cari..nama/no-po" name="search" value="{{request('search')}}">
                    ...
                    @foreach ($order as $key => $data)
                        <tr>
                            ...
                            <td>
                                ...
                                <a href="/pemesanan/detail/{{$data->id}}" class="btn btn-sm btn-success"><i class="nav-icon fas fa-eye"></i></a>
                                @endif
                                @if(in_array(auth()->user()->role,[1,2]))
                                <a href="/pemesanan/edit/{{$data->id}}" class="btn btn-sm btn-warning"><i class="nav-icon fas fa-edit"></i></a>
                                @endif
                                @if ($errors->any())
                                ...
                                @endif
                                @if(in_array(auth()->user()->role,[1,2]))
                                <form action="/pemesanan/destroy/{{$data->id}}" method="POST" class="d-inline">
                                    @method('delete')
                                    @csrf
