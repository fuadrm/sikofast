    ...
    <div class="container-fluid">
      ...
      <div class="card card-default">
        <div class="card-header" >
                ...
                {{-- <a href="/pembayaran/generateExcel" class="btn btn-success float-lg-right mr-1">ExportExcel</a> --}}
                ...
                <form class="form-inline ml-3" action="/pembayaran" >
                    <div class="input-group input-group-sm float-lg-left">
                      <input class="form-control form-control-navbar" type="search" 
                        placeholder="cari..nomor nota.." name="search" value="{{request('search')}}">
            ...
            <tbody>
                ...
                @foreach ($invoice as $key =>$data)
                    <tr>
                        ...
                        <td><label class="label {{ ($data->status_inv == 1) ? 'label-success':'label-danger'}}">{{ ($data->status_inv == 1) ? 'Lunas' : 'Down Payment' }}</label></td>
                        ...
                        <td>
                            @if($data->status_inv == 1)
                                @if(in_array(auth()->user()->role,[1,3]))
                                <a href="/pembayaran/status/{{$data->id}}" class="btn btn-sm btn-danger">Belum Lunas</a>
                                @endif
                            @else
                                @if(in_array(auth()->user()->role,[1,3]))
                                <a href="/pembayaran/status/{{$data->id}}" class="btn btn-sm btn-success">Lunas</a>
                                @endif
                            @endif
                        </td>
                        @endif
                        <td>
                            <a href="/pembayaran/detail/{{$data->id}}" class="btn btn-sm btn-success"><i class="nav-icon fas fa-eye"></i></a>
                            @if(in_array(auth()->user()->role,[1,3]))
                            <a href="/pembayaran/edit/{{$data->id}}" class="btn btn-sm btn-warning"><i class="nav-icon fas fa-edit"></i></a>
                            @endif
                            @if ($errors->any())
                            ...
                            @endif
                            @if(in_array(auth()->user()->role,[1,3]))
                            <form action="/pembayaran/destroy/{{$data->id}}" method="POST" class="d-inline">
                                @method('delete')
                                @csrf
